...
class UDaddy(Traffic):
    def __init__(self, logger, server, client, s_ip, c_ip, is_ipv6=False, server_args='', client_args=''):
        super(UDaddy, self).__init__(logger=logger, server=server, client=client)
        self._s_ip = s_ip
        self._c_ip = c_ip
    ...
    def get_server_command(self):
        if self._is_ipv6:
            return "udaddy -b %s %s" % (self._s_ip, self._server_args)
        else:
            return "udaddy %s" % (self._server_args)

    def get_client_command(self):
        if self._is_ipv6:
            return "udaddy -b %s -s %s %s" % (self._c_ip, self._s_ip, self._client_args)
        else:
            return "udaddy -s %s %s" % (self._s_ip, self._client_args)
